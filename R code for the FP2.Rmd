...
forecast_vix = function (model_code, start_date, subset_duration, 
  ...
  for (i in (1:n_steps)) {
    ...
    #saveRDS(r_fit, file = file_name, compress = "xz") 1 # commented to print out the code
...
```{r #+-+# exploratory data analysis SP_500/ VIX/ rv joint DF creation}
...
n_steps = 1 # changed from 10 for to print out the code
...
subset_duration = 6 # in months, incl 1 extra month for T_out (validation)
step_size = 2 # in months
n_steps = 1 # number of different time windows (to build different models) # changed from 10 for to print out the code
T_out <- 9 # forecast depth, days
...
```{r chain of forecasts RealGARCH11}
