...
temp = SP_500_corr_df[,c("date","sd_9d","sd_30d", "sd_3m", "sd_6m")]
names(temp) = c("date","S&P-500 SD_9d","S&P-500 SD_30d", "S&P-500 SD_3m", "S&P-500 SD_6m")
temp_long = melt(temp, id="date")
ggplot(temp_long, aes(y = value, x = variable)) + 
  geom_violin(trim = F, show.legend = F, fill = "steel blue", alpha = .1) + 
            geom_boxplot(width=0.21, fill="steel blue", outlier.color="steel blue", alpha = .5,
                         outlier.shape=3, color="steel blue", outlier.size=1) + 
...
n_steps = 9 # changed from 10 for output
...
```{r gelman.diag geweke.diag}

library(coda)
library(DrBats)

setwd('/Users/AM/Documents/_CU Masters/2020 fall Bayesian_7393/Final_Project/output')
temp_fit = readRDS("GARCH11 2020-11-27 for 2018-01-22 2018-07-22 .rda")
coda_fit <- coda.obj(temp_fit)
mc = mcmc.list(coda_fit)
...
##...## try different distribution (0,1) for x instead of N(0,1) in the realGarch


```

```{r other indexes return}
# do directly model SP500 return ~ rv, volume (not VIX as in the project)
  # functions to change 
    # quality of fit (now it compares with vix_9d)
    # forecast_vix
    # ...?
...
```{r STAN model RealGARCH(1,1) was working}
##++## analyze correlation RV and VIX for the same date #they almost repeat each other
##++## choose what to model (RV point or f(RV[interval])?) # return
##++## read papers about prediction of rv (Shepard and Shepard? etc)
##++## add log lik
...
data {
  int<lower=2> T; 
  vector[T] y;
  vector[T] rv;                 // it is x in HH2012 (realized volatility estimate)               
  real<lower=0> sigma_1;
  int<lower=0> T_out;
}
parameters {
  real mu;
  real<lower=0>           alpha0;   // it is omega in HH2012
  real<lower=0, upper=1>   beta1;    // it is the same beta1 in HH2012
