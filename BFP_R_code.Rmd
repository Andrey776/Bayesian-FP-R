...
quality_of_fit = function(fit, T_out, end_date, subset, subset_long) {
  ...
  qf = list(qf_df, summary(fit), waic(log_lik), loo(log_lik, r_eff = r_eff))
  (names(qf) = c("returns", "summary", "waic", "looic"))
  return(qf)
...
forecast_vix = function (model_code, start_date, subset_duration, 
  ...
  for (i in (1:n_steps)) {
    ...
    stan_dat = list(T = T, rv = rv, y = r, sigma_1 = sigma_1, T_out = T_out, v = v)
    ...
    qf = quality_of_fit(fit = r_fit, T_out = T_out, end_date = t_1, 
                        subset = subset, subset_long = subset_long)
    qf_list[[i]] <- qf
  }
  return(qf_list)
}  
...
temp = forecast_vix(model_code = model_code, start_date = start_date, 
                    subset_duration = subset_duration, 
                    step_size = step_size,n_steps = n_steps, T_out = 9, 
                    sigma_1 = 0.1, iter = iter, chains = chains) 
