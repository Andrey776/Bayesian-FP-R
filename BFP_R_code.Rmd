...
stan_dat_AR1 = list(T=T, y = r, T_out = T_out, v = v)
...
log_lik = extract_log_lik(r_fit_AR1, merge_chains = FALSE)
...
```{r quality of fit}
quality_of_fit = function(fit, T_out, end_date) {
  for (i in 1:T_out) {
...
quality_of_fit(fit = r_fit_AR1, T_out=3, end_date = end_date)
```
...
```{r #...# create subset and realGarch prediction}
...
subset_size = 5 # in months
step_size = 3 # in months
n_steps = 1 # 
T_out <- 7 # forecast depth, days
sigma_r_1 <- 0.1 # it is sqrt(h[1])

for (i in (1:n_steps)) {}
  start_date = as.Date(start_date) + months((i-1) * step_size)
  end_date = as.Date(start_date) + months(subset_size) 
  subset = build_vix9_rv_subset(start_date, end_date)
  T <- length(subset$vix_lin_ret)-1
  r <- subset$vix_lin_ret[-(c(1))]
  x <- subset$rv5[-(c(1))]
  file_name = paste("RG", as.character(Sys.Date()),
                    "for", as.character(start_date), as.character(end_date), ".rda", sep=" ")
  
  stan_dat_real_garch = list(T = T, x = x, r = r, h_1 = (sigma_r_1)^2, T_out = T_out)
  
  r_fit = stan(model_code = real_garch, data = stan_dat_real_garch, 
               iter = 3*10^3, chains = 3,
               control = list(max_treedepth = 16))
  
  setwd('/Users/AM/Documents/_CU Masters/2020 fall Bayesian_7393/Final_Project/data')

  save(r_fit, file = file_name, compress = "xz")
...
# WAIC and LOOC
log_lik = extract_log_lik(r_fit, merge_chains = FALSE)
r_eff = exp(relative_eff(log_lik))
